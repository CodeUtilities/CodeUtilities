...
import io.github.codeutilities.features.commands.search.codeaction.Action;
import io.github.codeutilities.features.commands.search.codeaction.ActionDump;
import io.github.codeutilities.features.commands.search.codeaction.Types;
...
public class SearchCommand implements Command {
    ...
    public void register(CommandDispatcher<FabricClientCommandSource> cd) {
        ...
        cd.register(literal("search")
                .executes(ctx -> { // /search clear shortcut
                    ChatUtil.sendMessage(new TranslatableText("command.codeutilities.search.cleared"), ChatType.SUCCESS);
                ...
                .then(argument("action", StringArgumentType.greedyString())
                        .executes(ctx -> { // /search <action..>
                            try {
                                ...
                                mc.player.sendMessage(TextUtil.colorCodesToTextComponent("§x§0§0§b§5§f§c✎ §x§0§0§e§0§b§0" + new TranslatableText("command.codeutilities.search.begin_search", "§x§0§0§f§8§f§c" + query + "§x§0§0§e§0§b§0").parse(mc.player.getCommandSource(), mc.player, 1).getString()).shallowCopy(), false);
                                ...
                                ChatUtil.sendMessage(new TranslatableText("command.codeutilities.search.invalid", ctx.getArgument("action", String.class)).parse(mc.player.getCommandSource(), mc.player, 1), ChatType.FAIL);
                            }
                            return 1;

                        })));

        cd.register(literal("exactsearch")
                .then(argument("codeblock", StringListArgumentType.string(allcodeblocks))
                        .then(argument("action", StringArgumentType.greedyString())
                                .executes(ctx -> { // /exactsearch <codeblock> <action..> (mostly used for the clickevent)
                                    try {
                                        ...
                                        //DFInfo.isOnDF() && DFInfo.currentState.getMode() == State.Mode.DEV && mc.player.isCreative()
                                        if (PlayerState.getMode() == PlayerState.Mode.DEV && mc.player.isCreative()) {
                                            CodeSearcher.beginSearch(searchType, actionArgument);
                                        }else {
                                            ChatUtil.sendMessage(new TranslatableText("command.codeutilities.require_dev_mode", ctx.getArgument("action", String.class)).parse(mc.player.getCommandSource(), mc.player, 1), ChatType.FAIL);
                                        }

                                    }catch (Exception e) {
                                        e.printStackTrace();

                                        ChatUtil.sendMessage(new TranslatableText("command.codeutilities.search.invalid").parse(mc.player.getCommandSource(), mc.player, 1), ChatType.FAIL);
                                    }
                                    return 1;
