...
public class ComponentUtil {
    ...
    public static TextComponent fromString(String message) {
        ...
        try {
            ...
            while (matcher.find()) {
                ...
                if (text.length() != 0) {
                    Component t = new TextComponent(text);
                    t.toFlatList(s);
                    result.append(t);
                }
                ...
                if (col.length() == 2) {
                    s = s.applyFormats(ChatFormatting.getByCode(col.charAt(1)));
                } else {
    ...
    public static String toFormattedString(Component message) {
        ...
        for (Component sibling : message.getSiblings()) {
            result.append(toFormattedString(new TextComponent(sibling.toString())));
        }
